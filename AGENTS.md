...
- `tests/` contains Playwright specs grouped by feature (for example `tests/client-management/client-management.spec.ts`).
- `tests/fixtures/test-fixtures.ts` holds shared selectors, helpers, login, and the screenshot-on-pass hook.
...
- `BO/` stores screenshots captured on passing tests at `BO/<test-case-name>/pass-<project>.png`.
...
- To run only a spec: `npx playwright test tests/client-management/client-management.spec.ts --project=chromium --headed`.
...
- Spec files follow `tests/<area>/<area>.spec.ts`.
...
- Passing tests auto-save screenshots under `BO/<test-case-name>/`.
...
- Test credentials and base URL live in `tests/fixtures/test-fixtures.ts`.
